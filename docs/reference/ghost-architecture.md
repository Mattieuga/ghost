...
- **Sharing works like a shared document.** One link per item, off or view or edit, that the owner can copy again any time; the token is readable by the owner alone, is looked up by hash when redeemed, and travels in the URL fragment so it never reaches a request log. Access that came through the link follows the link: a membership made by redeeming records it, turning the link off removes those memberships, and changing its role changes theirs, while people shared with by email are untouched.
...
- Triggers on `cloud_items` and `cloud_memberships` broadcast a `changed` event: on `ghost-tree:<workspace_id>`, which only the workspace's owner may listen to, and on `ghost-user:<user_id>` for the owner of each membership on the changed item or a folder above it, so a member hears about what is shared with them without seeing the rest of the workspace. A helper called from a policy must stay executable by `authenticated`; a revoked helper fails the policy closed.

Clients change data only through security-definer RPCs, all grouped by prefix: `cloud_ensure_workspace`, `cloud_create_item` and `cloud_adopt_items` (client-supplied IDs, idempotent, server-side rename on collision), `cloud_rename_item`, `cloud_move_item`, `cloud_duplicate_item`, `cloud_trash_item`; `cloud_document_role`, `cloud_document_heads`, `cloud_upload_document_versions`; and the sharing family `cloud_share_item`, `cloud_revoke_access`, `cloud_accept_invitations`, `cloud_set_share_link`, `cloud_redeem_share_link`, `cloud_leave_item`, `cloud_list_visible_items`, `cloud_item_sharing`, `cloud_set_display_name`. One Edge Function, `share-invite`, emails a person invited by address; it mails only an address that holds a pending invitation on the item, takes the role and the item's name from the database, uses Supabase's invite email for new accounts, and does nothing yet for existing ones. Realtime uses one broadcast topic per document, `ghost-cloud:<documentId>`, with the same role checks on who may listen and who may send.
