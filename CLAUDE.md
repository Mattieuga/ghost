...
- **Books are read-only viewers.** EPUB uses lazy EPUB.js; MOBI/AZW3/FB2 use pinned Foliate parsers and pagination with shared controls and offline script-free chapters. Native reads are bounded; reading position is local UI state. CBZ/CBR use the existing per-entry archive preview cache and image inspection, releasing temporary pages on navigation. Never rewrite or extract books beside their source. See `docs/adrs/2026-09-06-additional-book-formats.md` and the EPUB ADR.
