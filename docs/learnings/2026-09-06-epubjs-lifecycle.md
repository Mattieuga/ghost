...
A contents selection is another focus handoff: the select can retain focus, or disabling it during navigation can move focus to the body. Pass the initiating select to navigation and return focus to the stable reader when the chapter finishes, provided focus still belongs to that select or has fallen back to the body. The integration check now focuses the dropdown, commits a chapter, and immediately presses an arrow without clicking the reading surface.
